...
type GamePlayer struct {
	...
	Role              string    `json:"role" gorm:"-"`          // role 字段不在API中返回，用 gorm:"-" 忽略
	RoleRaw           string    `json:"-" gorm:"column:role"`   // 数据库中的实际角色
...
// AfterFind 在查询后处理角色显示逻辑
func (p *GamePlayer) AfterFind(tx *gorm.DB) error {
	p.Role = p.RoleRaw
	return nil
}
...
type GamePhase struct {
	...
	CreatedAt      time.Time  `json:"created_at"`
	KilledPlayer   *uint      `json:"killed_player"`    // 狼人杀的人
	SavedPlayer    *uint      `json:"saved_player"`     // 女巫救的人
	PoisonedPlayer *uint      `json:"poisoned_player"`  // 女巫毒的人
	CheckedPlayer  *uint      `json:"checked_player"`   // 预言家查验的人
	CheckedResult  string     `json:"checked_result"`   // 查验结果
	VotedPlayer    *uint      `json:"voted_player"`     // 投票放逐的人
	HunterVotedOut *uint      `json:"hunter_voted_out"` // 猎人被放逐
	WitchHasSave   bool       `json:"witch_has_save"`   // 女巫是否还有解药
	WitchHasPoison bool       `json:"witch_has_poison"` // 女巫是否还有毒药
}
